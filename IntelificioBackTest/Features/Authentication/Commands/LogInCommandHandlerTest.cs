...
public class LogInCommandHandlerTest
{
    ...
    {
        var mapperConfig = new MapperConfiguration(config => { config.AddProfile<UserProfile>(); });
        _mapper = new Mapper(mapperConfig);
        _userManager = UserManagerMock.CreateUserManager();
        _signInManager = UserManagerMock.CreateSignInManager(_userManager.Object);
    ...
    public async Task Handle_FailedLogIn_EmailNotConfirmed()
    {
        ...
        _ = _userManager
            .Setup(x => x.FindByNameAsync(It.IsAny<string>()))
            .ReturnsAsync(user);
        _ = _userManager.Setup(x => x.IsEmailConfirmedAsync(user)).ReturnsAsync(false);
        ...
        //Assert
        result.IsFailure.Should().BeTrue();
        result.Error.Should().Be(AuthenticationErrors.EmailNotConfirmed);
    }
    ...
    public async Task Handle_FailedLogIn_UserBlocked()
    {
        ...
        _ = _userManager
            .Setup(x => x.FindByNameAsync(It.IsAny<string>()))
            .ReturnsAsync(user);
        _ = _userManager.Setup(x => x.IsEmailConfirmedAsync(user)).ReturnsAsync(true);
        _ = _userManager.Setup(x => x.IsLockedOutAsync(user)).ReturnsAsync(true);
        ...
        //Assert
        result.IsFailure.Should().BeTrue();
        result.Error.Should().Be(AuthenticationErrors.UserBlocked);
    }
    ...
    public async Task Handle_FailedLogIn_InvalidPassword()
    {
        ...
        _ = _userManager.Setup(x => x.IsEmailConfirmedAsync(user)).ReturnsAsync(true);
        _ = _userManager.Setup(x => x.IsLockedOutAsync(user)).ReturnsAsync(false);
        _ = _signInManager
        ...
        //Assert
        result.IsFailure.Should().BeTrue();
        result.Error.Should().Be(AuthenticationErrors.WrongPassword);
    }
    ...
    public async Task Handle_SuccessLogIn()
    {
        ...
        _ = _userManager.Setup(x => x.GetRolesAsync(user)).ReturnsAsync(new List<string>() { "Admin" });
        ...
        var data = result.Response.Data as LoginCommandResponse;
    ...
    private void ConfigureConfiguration(Mock<IConfiguration> _configuration)
    {
        ...
        _ = _configuration.Setup(c => c.GetSection("Jwt:TokenExpirationInMinutes"))
            .Returns(tokenExpirationSectionMock.Object);
        _ = _configuration.Setup(c => c.GetSection("Jwt:RefreshTokenExpireInMinutes"))
            .Returns(refreshTokenExpirationSectionMock.Object);
