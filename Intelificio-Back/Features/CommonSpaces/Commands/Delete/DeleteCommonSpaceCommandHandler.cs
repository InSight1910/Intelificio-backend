...
public class DeleteCommonSpaceCommandHandler : IRequestHandler<DeleteCommonSpaceCommand, Result>
{
    ...
    {
        var space = await _context.CommonSpaces.Where(x => x.ID == request.Id).Include(x => x.Reservations)
            .FirstOrDefaultAsync(cancellationToken);
        if (space == null) return Result.Failure(CommonSpacesErrors.CommonSpaceNotFoundOnDelete);
        if (space.Reservations.Any(
                x => x.Status == ReservationStatus.CONFIRMED || x.Status == ReservationStatus.PENDING))
            return Result.Failure(CommonSpacesErrors.HasPendingReservationsOnDelete);
